...
router.post('/event', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({event,message:"Even added"});
    } catch (e) {
...
// Get events created by a particular user (GET request)
router.get('/events/:user', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Error retrieving events for the user.' });
    }
});
